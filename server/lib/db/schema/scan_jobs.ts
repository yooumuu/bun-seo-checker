...
export const seoMetrics = pgTable(
  'seo_metrics',
  {
    ...
    h1: text('h1'),
    h1Score: integer('h1_score'), // H1 质量评分 (0-100)
    h1Issues: text('h1_issues').array(), // H1 存在的问题列表
    ...
    htmlStructureScore: integer('html_structure_score'),
    htmlStructureIssues: jsonb('html_structure_issues'),
  },
  (table) => ({
...
export const linkMetrics = pgTable(
  'link_metrics',
  {
    id: serial('id').primaryKey(),
    pageId: integer('page_id')
...
export const insertScanPageSchema = createInsertSchema(scanPages).omit({
  id: true,
  createdAt: true,
});
...
export const insertLinkMetricsSchema = createInsertSchema(linkMetrics).omit({
  id: true,
});
